...
static inline IMP get_implementation (id receiver, Class class, SEL sel) {
    void *res;
...
inline IMP get_imp(Class class, SEL sel) {
...
__objc_forward (id object, SEL sel, arglist_t args)
{
     ...
     error:.  If it does then sent it. */
  {
    ...
    LogWriteSerial ("(%s) %s does not recognize %s",
	     (CLS_ISMETA (object->class_pointer)
	      ? "class"
	      : "instance" ),
             object->class_pointer->name, sel_get_name (sel));
    strcpy(msg, "__objc_forward had an error");
...
#ifdef OBJC_SPARSE2
  LogWriteSerial ("memory usage: (%s)\n", "2-level sparse arrays");
#else
  LogWriteSerial ("memory usage: (%s)\n", "3-level sparse arrays");
#endif

  LogWriteSerial ("arrays: %d = %ld bytes\n", narrays, 
	  (long) ((size_t) narrays * sizeof (struct sarray)));
  total += narrays * sizeof (struct sarray);
  LogWriteSerial ("buckets: %d = %ld bytes\n", nbuckets, 
	  (long) ((size_t) nbuckets * sizeof (struct sbucket)));
  total += nbuckets * sizeof (struct sbucket);

  LogWriteSerial ("idxtables: %d = %ld bytes\n",
	  idxsize, (long) ((size_t) idxsize * sizeof (void *)));
  total += idxsize * sizeof (void *);
  LogWriteSerial ("-----------------------------------\n");
  LogWriteSerial ("total: %d bytes\n", total);
  LogWriteSerial ("===================================\n");
