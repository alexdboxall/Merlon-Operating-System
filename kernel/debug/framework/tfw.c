...
static void GetHostState() {
    assert(sizeof(struct host_state) < 1024 - 8);

    uint8_t data[1024];
    memset(data, 0, 1024);
    data[0] = 0x11;
    DbgWritePacket(DBGPKT_TFW, data, 1);

    while (true) {
        int type;
        int size = 1024 - 32;
        DbgReadPacket(&type, data, &size);
        if (size < 1024 && type == DBGPKT_TFW) {
            ...
            assert(data[0] == 0x22);
            test_state = *((struct host_state*) (void*) (data + 8));
            break;
        }
    }
}
...
static void SetHostState(int code) {
    ...
    memset(data, 0, 1024);
    data[0] = code;
    memcpy(data + 8, &test_state, sizeof(test_state));

    if (code == 0x33) {
        strncpy((char*) (data + 8 + sizeof(test_state)), registered_tests[test_state.test_num].name, MAX_NAME_LENGTH);
    }

    DbgWritePacket(DBGPKT_TFW, data, sizeof(test_state) + 8 + (code == 0x33 ? MAX_NAME_LENGTH : 0));
    ReadAck();
...
void MarkTfwStartPoint(int id) {
    ...
    if (registered_tests[test_state.test_num].start_point == id) {
        ...
        registered_tests[test_state.test_num].code(registered_tests + test_state.test_num);
